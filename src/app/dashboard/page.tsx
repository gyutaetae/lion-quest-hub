...
import PointManager from "@/components/dashboard/PointManager";
import RecentPointFeed from "@/components/dashboard/RecentPointFeed";
...
const isCheckInRpcResult = (value: unknown): value is { success: boolean; message: string } => {
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const code = raw?.trim();
    if (!code) return;
    if (checkinHandledRef.current) return;
    ...
    (async () => {
      try {
        ...
        if (!isCheckInRpcResult(data)) throw new Error("출석 응답 형식이 올바르지 않습니다");
        toast({
          title: data.success ? "출석" : "출석 처리",
          description: data.message,
          variant: data.success ? "default" : "destructive",
        });
        if (data.success) await refreshProfile();
      } catch (e: unknown) {
        checkinHandledRef.current = false;
        const message = e instanceof Error ? e.message : "출석 처리 중 오류가 발생했습니다";
        toast({ title: "출석 처리 실패", description: message, variant: "destructive" });
      }
    })();
  }, [loading, user, profile, searchParams, router, toast, refreshProfile]);
